...
// 不建议重写Widget，而是VLCWidget直接继承Widget
// - Widget类主要负责UI布局
...
public:
    ...
    // 获得或微调 数据成员指针 / 数据成员状态
    // - 静态函数才可以访问私有静态数据成员
    static VLCWidget* get_VLCWidget_instance(void) { return m_VLCWidget_instance; }
    ...
    QStringList get_media_list() const { return m_media_paths; }                            // 获取媒体列表方法
    libvlc_time_t get_duration_time(void);
    HWND get_vlc_hand(void) const { return m_vlc_hwnd; }
...
signals:
    ...
    void hide_central_button();                 // 中间按钮隐藏
    void media_list_updated();                  // 媒体更新
...
public slots:
    ...
    void on_play_media_by_index(int index);        // 新增按索引播放媒体槽

private:
    // VLCWidget
    static VLCWidget            *m_VLCWidget_instance;         // 全局双击需要使用
...
//  libvlc_time_t               m_duration_seconds = 0;         // 视频总时长，目前是视频播放组vector，因此注释
    ...
    QStringList                 m_media_paths;                  // 存储媒体路径列表

    // windows相关成员变量
    UINT_PTR                    m_fullscreen_timer_id = 0;      // Windows定时器ID
    HWND                        m_vlc_hwnd = nullptr;           // VLC视频窗口句柄
    static HWND                 m_original_vlc_parent;          // 原窗口

    // Qt相关成员变量
    QWidget                     *m_video_widget = nullptr;      // vlc播放窗口
    QTimer                      *m_fullscreen_timer = nullptr;  // 全屏播放定时器
    bool                         m_is_fullscreen = false;       // 全屏状态标记，减少鼠标/ 键盘事件对于vlc窗口的依赖
