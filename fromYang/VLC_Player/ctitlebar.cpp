...
//    this->setStyleSheet("background-color:rgb(54,54,54)");
    ...
    _ptitleTextLabel->setStyleSheet(
                "QLabel{font-family: 'SimSun', '黑体';"
                "font-size:18px;"
                "color:black;}");

    // 新增播放列表按钮
    _pplaylistButton = new QPushButton(this);
    _pplaylistButton->setFixedSize(32, 32);
    _pplaylistButton->setIcon(QIcon(":/resources/titlebar/menu.svg"));
    _pplaylistButton->setIconSize(QSize(24, 24));
...
//                "background-image:url(:/resources/titlebar/set_hover.svg);border:none;}");
    ...
    _phLayout->addWidget(_pplaylistButton);  // 添加播放列表按钮
...
//    _phLayout->addWidget(_psetButton);
    ...
    connect(_pplaylistButton, &QPushButton::clicked, this, &CTitleBar::playlistButtonClicked);
...
{
    emit doubleClicked(); // 触发全屏信号
    QWidget::mouseDoubleClickEvent(event); // 保留原有行为（如窗口最大化）
}
