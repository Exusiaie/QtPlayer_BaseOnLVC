...
bool VLCWidget::play(const QStringList &names, void *hwnd)
{
    ...
    for(auto filename : names) {
        ...
        //libvlc_media_parse_with_options(m_media, libvlc_media_parse_local, 5000);  // 使用推荐的替代函数(第2个参数：指定是否解析媒体；第3个参数：默认超时时间)，但是这个函数反而不能获得正确的时间
...
//    CoInitialize(NULL);         // Windows平台兼容问题，初始化COM组件
    ...
    const char* vlc_args[] =
    {
        ...
        "--clock-jitter=50",    // 允许更大的时钟抖动（减少同步严格性）
        "--network-caching=300", // 增加网络缓冲（本地文件也适用）
    };
