...
void Widget::initialize_basic_UI()
{
    ...
    m_main_layout->addWidget(m_play_zone, 1);
    m_main_layout->addWidget(m_buttom_bar);
...
void Widget::connect_UI_controls()
{
    ...
    connect(m_buttom_bar->getVolumeButton()->getVolumeSliderDialog()->getSlider(), &QSlider::valueChanged, this, [=](int value)
        {
            int validValue = qBound(0, value, 100);
            ...
            m_buttom_bar->getVolumeButton()->setVolumeValue(validValue);
            m_buttom_bar->getVolumeButton()->setVolumeLabel(QString("音量: %1%").arg(validValue));
        }
    );
...
void Widget::connect_play_task_queue_signals()
{
    ...
    // 这里简单实现，假设在打开文件后更新
    connect(m_vlc_widget, &VLCWidget::hide_central_button, this, &Widget::update_play_task_queue);
    ...
    connect(m_vlc_widget, &VLCWidget::set_volume_slider_value, this, [=](int value)
        {
            ...
            m_buttom_bar->getVolumeButton()->setVolumeValue(value);
            m_buttom_bar->getVolumeButton()->setVolumeLabel(QString("%1%").arg(value));
        }
    );
...
// 新增更新播放队列槽
void Widget::update_play_task_queue()
{
    log_info("更新播放队列");
