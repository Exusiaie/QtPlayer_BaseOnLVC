...
//        _pvolumeSliderDlg->getSlider()->setAlignment(Qt::AlignCenter);
        _pvolumeSliderDlg->setSliderValue(_volumeValue);       // 设置初始音量

//        // 添加音量标签（方案1：在slider上方显示）
          // - 先不要添加，格式容易有问题
//        _volumeLabel = new QLabel(QString("音量, %1%").arg(_volumeValue), _pvolumeSliderDlg);
...
{
    log_info("CVolumeButton::mousePressEvent");
    if(event->button() == Qt::LeftButton)
    {
        ...
        if(_isMute) {   // 设置静音
            if(_pvolumeSliderDlg) {
                //保存原来的音量值
                _volumeValue = _pvolumeSliderDlg->getSlierValue();
