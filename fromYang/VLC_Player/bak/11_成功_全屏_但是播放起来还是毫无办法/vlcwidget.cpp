...
#include <QFileDialog>
#include <QMessageBox>
#include <QMouseEvent>     // 处理鼠标双击事件mouseDoubleClickEvent
...
// 静态变量初始化
VLCWidget* VLCWidget::m_VLCWidget_instance = nullptr;
HWND VLCWidget::m_original_vlc_parent = nullptr;
...
/*     Windows平台兼容问题，初始化COM组件
    ...
    CoInitialize(NULL);
*/
    m_VLCWidget_instance = this;    // 设置实例指针
    log_info("VLCwidget初始化完成");
}
...
VLCWidget::~VLCWidget()
{
    ...
    if (m_fullscreen_timer_id != 0)
    {
        KillTimer(NULL, m_fullscreen_timer_id);
        m_fullscreen_timer_id = 0;
    }
...
void VLCWidget::set_video_widget(QWidget *widget)
{
    ...
    if (m_video_widget && m_media_player) {
        ...
        if (vlcHwnd && m_original_vlc_parent == NULL) {
            m_original_vlc_parent = GetParent(vlcHwnd); // 记录原始父窗口
        }
    }
}
...
{
    // 获取VLCWidget实例（需要全局指针或信号槽传递，这里简化处理）
    VLCWidget *VLCWidget = VLCWidget::get_VLCWidget_instance();
    if(!VLCWidget || !VLCWidget->get_vlc_hand()) return;

    // 禁用VLC子窗口的事件拦截
    EnumChildWindows(VLCWidget->get_vlc_hand(), EnumVLC, NULL);
}
...
/*    // 舍弃vlc全屏切换与Qt，改为由windows API接管
    ...
    if (!is_fullscreen) {
        ...
        if (vlcHwnd && IsWindow(vlcHwnd)) {
            ...
            m_original_vlc_parent = GetParent(vlcHwnd);
        ...
        if (vlcHwnd && IsWindow(vlcHwnd)) {
            // 恢复原始父窗口（通常是桌面窗口）
            SetParent(vlcHwnd, m_original_vlc_parent);
            // 恢复窗口样式为顶层窗口
...
*/
    ...
    // windows api + 定时器
    log_info("mouseDoubleClickEvent已接收");

    // 检查窗口媒体播放器与句柄
    if(!m_media_player || !m_vlc_hwnd || !m_video_widget)
    {
        log_info("缺少必要组件, 返回");
        event->accept();
        return;
    ...
    HWND mainHwnd = (HWND)this->parentWidget()->winId();    // 获得父窗口句柄
    WINDOWPLACEMENT wp;
    wp.length = sizeof(WINDOWPLACEMENT);
    GetWindowPlacement(mainHwnd, &wp);

    if(m_is_fullscreen)     // 进入全屏
    {
        // 保存原始父窗口
        m_original_vlc_parent = GetParent(m_vlc_hwnd);

        // 设置vlc为子窗口
        SetParent(m_vlc_hwnd, (HWND)m_video_widget->winId());
        SetWindowLong(m_vlc_hwnd, GWL_STYLE, WS_CHILD | WS_VISIBLE);
    ...
    {
        // 恢复VLC窗口父窗口
        if(m_original_vlc_parent && IsWindow(m_original_vlc_parent))
        {
            SetParent(m_vlc_hwnd, m_original_vlc_parent);
            SetWindowLong(m_vlc_hwnd, GWL_STYLE, WS_OVERLAPPEDWINDOW | WS_VISIBLE);
        }
...
/*    // 退出全屏功能交给windows API实现
    if (event->key() == Qt::Key_Escape) {
        ...
        if (wp.showCmd == SW_SHOWMAXIMIZED) {
            ...
            HWND vlcHwnd = (HWND)libvlc_media_player_get_hwnd(m_media_player);
            if (vlcHwnd && IsWindow(vlcHwnd)) {
                SetParent(vlcHwnd, m_original_vlc_parent); // 使用上面记录的原始父窗口
                SetWindowLong(vlcHwnd, GWL_STYLE, WS_OVERLAPPEDWINDOW | WS_VISIBLE);
                RECT rect;
                GetClientRect((HWND)m_video_widget->winId(), &rect);
                SetWindowPos(vlcHwnd, NULL, 0, 0, rect.right - rect.left, rect.bottom - rect.top, SWP_NOZORDER);
...
//void VLCWidget::toggle_full_screen()
//{
//    libvlc_toggle_fullscreen(m_media_player);
//}

// 添加辅助函数调整VLC窗口大小
void VLCWidget::resize_vlc_window()
{
//    if (m_media_player && m_video_widget)
//    {
//        HWND vlcHwnd = (HWND)libvlc_media_player_get_hwnd(m_media_player);
...
//    }
    ...
    if(!m_vlc_hwnd) return;
    RECT rect;
    GetClientRect((HWND)m_video_widget->winId(), &rect);
    int width = rect.right - rect.left;
    int height = rect.bottom - rect.top;
    SetWindowPos(m_vlc_hwnd, NULL, 0, 0, width, height, SWP_NOZORDER | SWP_NOACTIVATE);
}
...
bool VLCWidget::play(const QStringList &names, void *hwnd)
{
    ...
    // 7. 获取VLC窗口句柄​
    m_vlc_hwnd = (HWND)libvlc_media_player_get_hwnd(m_media_player);
    // 启动定时器（300ms间隔）
    if (m_fullscreen_timer_id == 0)
    {
        m_fullscreen_timer_id = SetTimer(NULL, 1, 300, TimeProc);
    }
...
//    "--input-ignore-mouse-events", // 忽略鼠标事件（让事件传递给Qt）(同样不支持)

//    "--no-input-default-bindings", // 禁用默认输入绑定（包括双击全屏）(不支持)
